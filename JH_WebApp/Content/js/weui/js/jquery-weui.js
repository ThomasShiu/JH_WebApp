...
+ function ($) {
    ...
    //如果參數過多，建議通過 config 物件進行配置，而不是傳入多個參數。
    ...
    $.login = function (text, title, onOK, onCancel, username, password) {
        ...
        var modal = $.modal({
            ...
            '<input type="text" class="weui-input weui-prompt-input" id="weui-prompt-username" value="' + (config.username || '') + '" placeholder="輸入用戶名" />' +
            '<input type="password" class="weui-input weui-prompt-input" id="weui-prompt-password" value="' + (config.password || '') + '" placeholder="輸入密碼" />',
            title: config.title,
            autoClose: false,
            buttons: [
                {
    ...
    defaults = $.modal.prototype.defaults = {
        ...
        autoClose: true //點擊按鈕自動關閉對話方塊，如果你不希望點擊按鈕就關閉對話方塊，可以把這個設置為false
    };
...
+ function ($) {
    ...
    $.toast = function (text, style, callback) {
        ...
        show('<i class="' + iconClassName + ' weui-icon_toast"></i><p class="weui-toast_content">' + (text || "已經完成") + '</p>', className);
    ...
    $.showLoading = function (text) {
        ...
        html += '</div>';
        html += '<p class="weui-toast_content">' + (text || "數據載入中") + '</p>';
...
+function ($) {
    ...
    PTR.prototype.touchMove = function (e) {
        ...
        if (Math.abs(this.diffX) > Math.abs(this.diffY)) return true; // 說明是左右方向的拖動
...
+ function ($) {
    ...
    $.openPicker = function (tpl, className, callback) {
        ...
        //關於佈局的問題，如果直接放在body上，則做動畫的時候會撐開body高度而導致捲軸變化。
        var dialog = $(tpl).appendTo(container);

        dialog.width(); //通過取一次CSS值，強制流覽器不能把上下兩行代碼合併執行，因為合併之後會導致無法出現動畫。
...
+ function ($) {
    ...
    Select.prototype.parseInitValue = function () {
        ...
        //如果input為空，只有在第一次初始化的時候才保留默認選擇。因為後來就是使用者自己取消了全部選擇，不能再為他選中預設值。
    ...
    Select.prototype.open = function (values, titles) {
        ...
        // open picker 會默認關掉其他的，但是 onClose 不會被調用，所以這裡先關掉其他select
        for (var i = 0; i < selects.length; i++) {
            var s = selects[i];
            if (s === this) continue;
            if (s._open) {
                if (!s.close()) return false; // 其他的select由於某些條件限制關閉失敗。
            }
        }
        ...
        if (!force) {
            ...
            if (this.config.multi) {
                ...
                if (this.config.max !== undefined && this.data.length > this.config.max) {
                    $.toast("最多只能選擇" + this.config.max + "個", "text");
                    return false
                }
            }
        }
    ...
    defaults = $.fn.select.prototype.defaults = {
        ...
        beforeClose: undefined, // function 關閉之前，如果返回false則阻止關閉
...
+function ($) {
    ...
    var Calendar = function (params) {
        ...
        //iphone 上無法正確觸發 click，會導致點擊外面無法關閉
        ...
        p.close = function () {
            if (!p.opened || p.inline) return;
            p.animating = false;  //有可能還有動畫沒做完，因此animating設置還沒改。
            if (inPopover()) {
                $.closePicker(p.popover);
                return;
            }
...
+ function ($) {
    ...
    defaults = $.fn.datetimePicker.prototype.defaults = {
        ...
        monthSplit: '-',
        dateSplit: '',  // 默認為空
        datetimeSplit: ' ',  // 日期和時間之間的分隔符號，不可為空
        ...
        parse: function (str) {
            // 把字串轉換成陣列，用來解析初始值
            // 如果你的定制的初始值格式無法被這個預設函數解析，請自訂這個函數。比如你的時間是 '子時' 那麼預設情況這個'時'會被當做分隔符號而導致錯誤，所以你需要自己定義parse函數
            // 默認相容的分隔符號
...
+ function ($) {
    ...
    //Popup 和 picker 之類的不要共用一個彈出方法，因為這樣會導致 在 popup 中再彈出 picker 的時候會有問題。
...
+function ($) {
    ...
    Swipeout.prototype.touchStart = function (e) {
        ...
        this.diffX = this.diffY = 0;
        this._closeOthers()
        this.limit = this.container.find('>.weui-cell__ft').width() || 68; // 因為有的時候初始化的時候元素是隱藏的（比如在對話方塊內），所以在touchstart的時候計算寬度而不是初始化的時候
    };
    ...
    Swipeout.prototype.touchMove = function (e) {
        ...
        this.diffX = p.x - this.start.x;
        this.diffY = p.y - this.start.y;
        if (Math.abs(this.diffX) < Math.abs(this.diffY)) { // 說明是上下方向在拖動
            this.close()
            this.start = false
            return true;
        }
    ...
    Swipeout.prototype.touchEnd = function () {
        ...
        if (this.diffX < -5 && t < 200) { // 向左快速滑動，則打開
            this.open()
        } else if (this.diffX >= 0 && t < 200) { // 向右快速滑動，或者按一下,則關閉
            this.close()
